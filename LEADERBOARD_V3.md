...
截至 2026-08-06，本账号最佳 public LB 为 **0.97084**，排名 **42 / 约 770**；
当时第一名为 **0.97115**，差距 **0.00031**。本账号另外两个已完成提交为
**0.97083** 和 **0.96643**。public 榜只覆盖约 20% 测试集，最后排名由约 80%
private 榜决定，因此不应为了几百万分位的 public 提升破坏 CV 稳定性。
...
当前提交已在 Kaggle 页面完成格式校验与实际计分；截至 2026-08-06 的最佳 public
结果为 **0.97084 / Rank 42**。最终仍应保留 86 模型主候选与 79 模型稳健候选，避免
只依据约 20% public split 的微小差异选择两个高度重复的变体。
